...
class SignupForm(forms.Form):
    ...
    username = forms.RegexField(regex=USERNAME_RE,
                                max_length=30,
                                widget=BootstrapTextInput(attrs=attrs_dict),
                                label=_("Username"),
                                error_messages={'invalid': _('Username must contain only letters, numbers, dots and underscores.')})
    email = forms.EmailField(widget=BootstrapTextInput(attrs=dict(attrs_dict,
                                                               maxlength=75)),
                             label=_("Email"))
    password1 = forms.CharField(widget=BootstrapPasswordInput(),
                                label=_("Create password"))
    password2 = forms.CharField(widget=BootstrapPasswordInput(attrs=attrs_dict,
                                                           render_value=False),
...
class AuthenticationForm(forms.Form):
    ...
    password = forms.CharField(label=_("Password"),
                               widget=BootstrapPasswordInput(attrs=attrs_dict, render_value=False))
